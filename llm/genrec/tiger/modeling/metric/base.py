...
# NOTE on naming: NDCG*Metric below actually compute DCG (sum of discounted hits) without
# dividing by IDCG. With EXACTLY ONE ground-truth item per user (leave-one-out — the setup this
# repo uses), IDCG == 1, so DCG == NDCG numerically and the names are interchangeable HERE.
# If you move to multi-item ground truth, divide by the true IDCG before reporting "NDCG".
